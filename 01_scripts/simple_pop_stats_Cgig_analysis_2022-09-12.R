...
run_2022_08_04.keep <- missing_data_merged.df[missing_data_merged.df$keep=="old", "indiv"]
run_2022_10_07.keep <- missing_data_merged.df[missing_data_merged.df$keep=="new", "indiv"]
length(run_2022_08_04.keep)
length(run_2022_10_07.keep)

# Keep only the selected from each
obj.r_2022_08_04.best <- obj.r_2022_08_04.common[i=run_2022_08_04.keep]
indNames(obj.r_2022_08_04.best)

### PLAN for retaining the best, and evaluating shared genos ###
# [x] Find the percent missing per individual
# [x] Merge the two datasets based on the common sample ID, keeping them separate and keeping the number typed
### This may be a good place to also do a direct comparison of % shared genotypes

# [x] determine which of the two individuals to keep, create a keep vector

# Remove the separate pops, then drop individuals that are not the keep ind
# As a result, will have the best copy of the individual
### /END/ PLAN ###
